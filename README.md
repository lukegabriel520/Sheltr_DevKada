...
## Tech stack in brief
The user-facing app is built with Expo so it can run on phones and the web with the same codebase. The backend is a Python Flask API that handles requests, talks to routing services, and scores hazards. Evacuation centers are stored in Supabase, which is a hosted PostgreSQL database with geospatial features. Routing uses the Stadia or Valhalla engines to compute paths.

Hazard layers are stored as GeoJSON files, and geospatial scoring uses Python libraries like Shapely and Pandas. These map layers can be prepared or inspected in GIS software such as QGIS. The project can be deployed with Railway for the API and Vercel for the web build.
